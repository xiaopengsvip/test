...
import { Terminal, Shield, Zap, Globe, ArrowRight, BookOpen, BrainCircuit, MonitorSmartphone, Puzzle, Cpu, Clock, Copy, CheckCircle2, HeadphonesIcon, Download, LayoutDashboard, User, Blocks, Presentation, MessageSquare, Github, Command, TerminalSquare, Package, PlayCircle, Activity, LayoutTemplate } from 'lucide-react';
...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0A0A0B] text-slate-900 dark:text-slate-300 font-sans selection:bg-indigo-500/30 overflow-hidden transition-colors duration-300">
      ...
      <nav className="flex items-center justify-between px-4 md:px-6 py-4 border-b border-slate-200 dark:border-white/10 sticky top-0 z-40 bg-white/80 dark:bg-[#0A0A0B]/80 backdrop-blur-md transition-colors duration-300">
        ...
        <div className="flex items-center gap-3 md:gap-6">
          ...
          <button onClick={() => navigate('/docs')} className="hidden md:flex text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors items-center gap-2">
          ...
          >
            {isAuthenticated ? (
              <><LayoutDashboard className="w-4 h-4" /> 控制台</>
            ) : (
      ...
      <section className="relative px-4 md:px-6 py-16 md:py-32 max-w-6xl mx-auto text-center">
        ...
        <div className="relative z-10">
          ...
          >
            ...
            <button onClick={() => navigate('/docs')} className="w-full sm:w-auto flex items-center justify-center gap-2 px-8 py-4 bg-white dark:bg-white/5 text-slate-700 dark:text-white border border-slate-200 dark:border-white/10 rounded-xl font-semibold hover:bg-slate-50 dark:hover:bg-white/10 transition-all hover:-translate-y-1">
              <BookOpen className="w-4 h-4" /> 本站文档
            </button>
      ...
      <section className="px-6 py-24 border-t border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#0A0A0B] relative z-10 transition-colors">
        <div className="max-w-4xl mx-auto">
          ...
          >
            <div className="flex flex-col sm:flex-row border-b border-slate-200 dark:border-white/10">
              <button 
                onClick={() => setActiveTab('mac')}
                className={`flex-1 py-4 px-6 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${activeTab === 'mac' ? 'text-indigo-600 dark:text-white bg-slate-50 dark:bg-white/5 border-b-2 border-indigo-600 dark:border-indigo-500' : 'text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-white/5'}`}
              >
                <Command className="w-4 h-4" /> macOS / Linux
              </button>
              <button 
                onClick={() => setActiveTab('win')}
                className={`flex-1 py-4 px-6 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${activeTab === 'win' ? 'text-indigo-600 dark:text-white bg-slate-50 dark:bg-white/5 border-b-2 border-indigo-600 dark:border-indigo-500' : 'text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-white/5'}`}
              >
                <TerminalSquare className="w-4 h-4" /> Windows (PowerShell)
              </button>
              <button 
                onClick={() => setActiveTab('npm')}
                className={`flex-1 py-4 px-6 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${activeTab === 'npm' ? 'text-indigo-600 dark:text-white bg-slate-50 dark:bg-white/5 border-b-2 border-indigo-600 dark:border-indigo-500' : 'text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-white/5'}`}
              >
