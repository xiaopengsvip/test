      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className={`fixed inset-y-0 right-0 ${isFullscreen ? 'w-full' : 'w-[95vw] sm:w-[90vw] md:w-[80vw] lg:w-[70vw] xl:w-[60vw]'} bg-white dark:bg-[#0A0A0B] shadow-2xl z-50 flex flex-col border-l border-slate-200 dark:border-white/10 transition-all duration-300`}
          >
            {/* Header */}
            <div className="flex items-center justify-between p-4 md:p-6 border-b border-slate-200 dark:border-white/10 shrink-0 bg-white/80 dark:bg-[#0A0A0B]/80 backdrop-blur-md">
              ...
              <div className="flex items-center gap-2">
                ...
                <a 
                  ...
                  target="_blank" 
                  rel="noreferrer"
                  className="p-2 text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-slate-100 dark:hover:bg-white/5 rounded-lg transition-colors"
                  title="在新标签页中打开"
                >
            ...
            {/* Content */}
            <div className="flex-1 w-full bg-slate-50 dark:bg-[#0A0A0B] overflow-hidden relative">
              {['docs.openclaw.ai', 'github.com', 'discord.com', 'linktr.allapple.top'].some(domain => url.includes(domain)) || hasError ? (
                <div className="flex flex-col items-center justify-center h-full p-8 text-center">
                  <div className={`w-20 h-20 rounded-full flex items-center justify-center mb-6 border ${hasError ? 'bg-red-50 dark:bg-red-500/10 border-red-100 dark:border-red-500/20' : 'bg-indigo-50 dark:bg-indigo-500/10 border-indigo-100 dark:border-indigo-500/20'}`}>
                    {hasError ? (
