...
import { LogOut, LayoutDashboard, BookOpen, Terminal, Home, Menu, X, ChevronLeft, ChevronRight, Clock, Sun, Moon, MonitorSmartphone } from 'lucide-react';
    ...
    <>
      ...
      <aside 
        className={`
          ...
          bg-white dark:bg-[#0D0D0F] border-r border-slate-200 dark:border-white/10 flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.02)]
      ...
      >
        <div className="p-6 border-b border-slate-100 dark:border-white/10 flex items-center justify-between shrink-0 h-[88px]">
          ...
          {/* Mobile Close Button */}
          <button 
            className="md:hidden p-2 -mr-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 rounded-lg hover:bg-slate-50 dark:hover:bg-white/10 transition-colors"
            onClick={() => setIsMobileOpen(false)}
          >
            <X className="w-5 h-5" />
          </button>
        ...
        <div className="p-4 flex-1 overflow-y-auto overflow-x-hidden">
          ...
          <nav className="space-y-1">
            ...
            <NavItem icon={LayoutDashboard} label="控制台" to="/dashboard" />
            <NavItem icon={MonitorSmartphone} label="Mac/Linux 安装" to="/mac-linux-install" />
            <NavItem icon={Terminal} label="工具" to="/tools" />
            <NavItem icon={BookOpen} label="文档" to="/docs" />
          </nav>
        </div>

        <div className="p-4 border-t border-slate-100 dark:border-white/10 bg-slate-50/50 dark:bg-white/5 shrink-0">
          {/* Time Display */}
          ...
          <button
            ...
            className="hidden md:flex w-full items-center justify-center p-2 mb-4 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-200 dark:hover:bg-white/10 rounded-xl transition-colors"
          ...
          <button
            ...
            className={`flex items-center gap-3 px-3 py-2.5 mb-2 w-full text-left text-slate-600 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 rounded-xl font-medium transition-colors group ${isCollapsed ? 'justify-center' : ''}`}
          ...
          <button
            ...
            className={`flex items-center gap-3 px-3 py-2.5 w-full text-left text-slate-600 dark:text-slate-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-xl font-medium transition-colors group ${isCollapsed ? 'justify-center' : ''}`}
